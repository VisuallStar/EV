...
class ChatSession {
  ...
  Map<String, dynamic> toJson() => {'id': id, 'title': title, 'timestamp': timestamp.toIso8601String(), 'messages': messages};
...
class ChatHistoryService {
  ...
  static Future<void> saveSession(ChatSession session) async {
    try {
      ...
      if (index >= 0) sessions[index] = session; else sessions.insert(0, session);
      final jsonList = sessions.map((s) => s.toJson()).toList();
      await file.writeAsString(jsonEncode(jsonList));
    } catch (e) { print('Error saving chat session: $e'); }
  }
