...
class SkillMemoryService {
  ...
  Future<void> _loadSkills() async {
    ...
    try {
      ...
      if (!await file.exists()) { _isLoaded = true; return; }
      final lines = await file.readAsLines();
      _skills = lines.where((line) => line.trim().isNotEmpty).map((line) => SavedSkill.fromJson(jsonDecode(line) as Map<String, dynamic>)).toList();
      _isLoaded = true;
    } catch (e) {}
  }
  ...
  Future<SavedSkill?> findSkill(String taskGoal) async {
    ...
    SavedSkill? bestMatch; double highestSim = 0.0;
    for (final skill in _skills) {
      final sim = _jaccardSimilarity(queryKeywords, skill.taskKeywords);
      if (sim > highestSim) { highestSim = sim; bestMatch = skill; }
    }
    return highestSim > 0.6 ? bestMatch : null;
  }
  ...
  Future<void> saveSkill(String taskGoal, List<ActionStep> steps) async {
    ...
    for (final skill in _skills) {
      if (_jaccardSimilarity(queryKeywords, skill.taskKeywords) > 0.8) {
        skill.successCount++; skill.lastUsed = DateTime.now();
        if (steps.length < skill.steps.length) { skill.steps.clear(); skill.steps.addAll(steps); }
        await _saveAllSkills(); return;
      }
    }
    _skills.add(SavedSkill(id: DateTime.now().millisecondsSinceEpoch.toString(), task: taskGoal, taskKeywords: queryKeywords, successCount: 1, failCount: 0, lastUsed: DateTime.now(), steps: steps));
  ...
  Future<void> recordFailure(String skillId) async {
    ...
    if (index != -1) { _skills[index].failCount++; await _saveAllSkills(); }
  }
}
