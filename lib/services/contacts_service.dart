...
class ContactsService {
  Future<List<Contact>> searchContacts(String query) async {
    ...
    return contacts.where((c) => c.displayName.toLowerCase().contains(lowerQuery)).toList();
  }
  ...
  Future<String> searchAndFormat(String query) async {
    ...
    for (final contact in contacts.take(5)) {
      ...
      if (contact.phones.isNotEmpty) buffer.write(' - ${contact.phones.first.number}');
      if (contact.emails.isNotEmpty) buffer.write(' - ${contact.emails.first.address}');
      buffer.writeln();
    }
    if (contacts.length > 5) buffer.writeln('...and ${contacts.length - 5} more');
